$(document).ready(() => {
    ...
    $.ajax({
      ...
      success(json) {
        ...
        for (let i = 0; i < json.length; i += 1) {
          if (json[i][3].split('-')[2] < 10) data.push(json[i][3].split('-')[2].substring(1, 2));
          else data.push(json[i][3].split('-')[2]);
        }
        for (let i = 0; i < json.length; i += 1) {
          console.log(data[i]);
          $(`#${data[i]}`).append(`<li id="${json[i][0]}">${json[i][1]}</li>`);
          $(`#${data[i]} > [id=${json[i][0]}]`).click(() => {
            alert(`내용 : ${json[i][2]}`);
          });
        }
      },
  ...
  function buildCalendar() {
    ...
    for (let i = 1; i <= lastDate.getDate(); i++) {
      // 1일부터 마지막 일까지 돌림
      const cell = row.insertCell();// 열 한칸한칸 계속 만들어주는 역할
